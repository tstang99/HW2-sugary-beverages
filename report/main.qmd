...
There are missing entries in this dataset, with a significant portion of the missing data attributed to the 'Juice100,' 'Ojuice,' and 'Sports' columns. Given that neither these measurements themselves nor their links with the consumption of ZeroCal/Sugary beverages is not in the main interest of study, the missing values in these columns should not be specifically focused. Additionally, there are seven entirely missing consecutive entries at HF hospital during the follow-up period. This is possible due to the hospital's cafeteria and convenient shop close down for renovation. Apart from these, there are two isolated cases of missing entries. These can be reasonably attributed to temporary closures for public holidays.
...
There are three different types of generalized linear models fitted in this report, adapted for different structures of data. The first model (Equation \ref{eq1}) is GLMM with Poisson distribution, which is suitable for count data where the counts are grouped by random effects. In this case, the random effects are Site and Total.

\begin{equation}
\label{eq1}
\log(\lambda_{ij}) = \beta_0 + \beta_1 \cdot \text{Intervention}_i + u_{j} + v_{i}
\end{equation}

where $\lambda_{ij}$ is the expected count for the i-th level of Intervention and the j-th level of Site and Total, $\beta_0$ is the intercept, $\beta_1$ is the coefficient for the intervention, $u_j$ is the random effect for the j-th Site, and $v_i$ is the random effect for the i-th level of Total. The random effects are assumed to follow a Poisson distribution. This model assumed the expected number of zero calorie beverage sales to have poisson distribution, where its expectation should be equal to its variance. When such an assumption is violated, usually quasi-maximum likelihood estimation is used. This method estimates the regression parameters without the need to specify a distribution for the outcome.

The second model (Equation \ref{eq2}) is similar to the first but assumes only a random effect on Total. Notice that $\beta_2$ is the coefficient for Site as a fixed effect.

\begin{equation}
\label{eq2}
\log(\lambda_{ij}) = \beta_0 + \beta_1 \cdot \text{Site}_i + \beta_2 \cdot \text{Intervention}_i + v_{j}
\end{equation}

The third model (Equation \ref{eq3}) is a GLMM with negative binomial distribution. Similar to the first model, it is also used for count data, but is more suitable when there is overdispersion (the variance is greater than the mean).
