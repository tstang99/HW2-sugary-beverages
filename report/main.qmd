...
```{r figs2, warnings = FALSE, echo=FALSE, message=FALSE, fig.width=7,fig.height=4,fig.cap="\\label{fig:figs2}Coefficient Plot"}
library(gridExtra)
...
sugary_glm_result <- data.frame(
  site = rep(c('chop', 'HF', 'NS'), 5),
  intervention = rep(c('both', 'cal', 'dis', 'dismes', 'exer'), each = 3),
  percentage_effect = c(19.7987, 11.2814,
               -16.3545, 29.2919, 8.2066, -17.8256, 29.7091, -13.2208,
               -10.1400, 26.3538, -14.2257, 58.3786, 23.4858, 1.7126,
               9.1903),
  se = c(10.4370, 10.5273,
                 10.5756, 10.4323, 10.5238, 10.5937, 10.4387, 10.5700,
                 10.5765, 10.4378, 10.5623, 10.4731, 10.4383, 10.6071,
                 10.6241),
  p_value = c(0.05783, 0.28389, 0.12200, 0.00499, 0.43550, 0.09244, 0.00443,
  0.21101,0.33770,0.01157, 0.17803, 2.49e-08, 0.02445, 0.87173, 0.38701))


p2 = ggplot(sugary_glm_result, aes(x = intervention, y = percentage_effect, fill = site)) +
  geom_bar(stat='identity', position=position_dodge(width=0.8), width=0.7) +
  geom_errorbar(aes(ymin = percentage_effect - se, ymax = percentage_effect + se),
                position=position_dodge(width=0.8), width=0.25) +
  scale_fill_brewer(palette="Pastel1") +
...
\end{verbatim}
```
From the above plots, we conclude that the intervention methods have different effects at the three sites. The differing impacts of interventions across sites may suggest underlying demographic differences. These could be in terms of age, income, health consciousness, or other socio-cultural factors that influence purchasing behaviors. This highlights the necessity for customized strategies that consider the unique demographic and behavioral profiles of different populations.
